...
table.style.display = 'document.getElementById("Meals")';
...
<script>
    ...
    // veiw details
function viewMealsDetails(id){
    $('#hiddendata').val(id);
    console.log(" before sending data to API, id value=",id);
var url = 'https://iqbetspro.com/pg-management/GET-API-Meals-View.php';
 var data = { id: id };
...
$('#breakfast').val(userid[0].breakfast);
$('#lunch').val(userid[0].lunch);
$('#floor_no').val(userid[0].floor_no);
$('#room_no').val(userid[0].room_no);
$('#bed_no').val(userid[0].bed_no);
$('#dinner').val(userid[0].dinner);
//$('#date').val(userid[0].date.replace(/-/g,"/"));


console.log(userid[0].date);
...
//$('#resolve_date').val(userid[0].resolve_date);
